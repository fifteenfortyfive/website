        ...
        <div class="hover-card">
          <div class="verti-flex">
            <div class="has-margin-bottom-sm">
              ...
              <p><strong>{pb}<span class="has-text-grey-light"> / {est}</span></strong></p>
              <p>{game.name}</p>
            </div>
            ...
            <div class="has-margin-bottom-sm">
              ...
              { pairs.length > 0
                ? <p>Pair With:
                    <ul>
                      { _.map(pairs, (pair) => <li>&ndash; {pair}</li>) }
                    </ul>
                  </p>
                : <p><em>No pairing requests</em></p>
              }
              { avoids.length > 0
                ? <p>Avoid:
                    <ul>
                      { _.map(avoids, (avoid) => <li>&ndash; {avoid}</li>) }
                    </ul>
                  </p>
                : <p><em>No avoid requests</em></p>
              }
            </div>
